...
export class PollDurableObject {
	...
	private async loadPoll(txn: { get: <T = unknown>(key: string) => Promise<T | undefined> } = this.state.storage): Promise<PollData> {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (method === 'POST' && url.pathname === '/vote') {
			...
			await this.state.storage.transaction(async txn => {
				...
				await Promise.all([txn.put('poll', poll), txn.put('voters', voters)]);
