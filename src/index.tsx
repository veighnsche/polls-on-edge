...
app.get("*", jsxRenderer(({ children }) => <Layout>{children}</Layout>));
...
const validatePollForm = async (c: Context, next: Next) => {
  ...
  const options = [1, 2, 3, 4, 5]
    ...
    .filter(Boolean);
  const ttl = String(body["ttl"] ?? "86400");
  ...
  if (!parsed.success) {
    return c.html(
      <section className="rounded-xl shadow p-10 flex flex-col items-center bg-card max-w-lg mx-auto mt-10">
        ...
        <pre className="text-sm text-destructive/70">
...
app.post("/api/poll/:pollId/edit", validatePollForm, async (c) => {
  ...
  const { question, options, ttl } = c.get("pollForm");
  const jwtPayload = c.get("jwtPayload");
  const result = await pollService.editPoll({ pollId, question, options, ttl, env: c.env, jwtPayload });
  if (!result.ok) {
...
app.post(
  ...
  zValidator("json", VoteRequestBodySchema),
  async (c) => {
    const pollId = c.req.param("pollId");
